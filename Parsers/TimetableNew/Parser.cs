...
namespace Parsers.TimetableNew
{
    public class Parser
    {
            ...
            foreach (var content in filesContents)
            {
                ...
                for (int i = 0; i < lines.Length; i++)
                {
                    ...
                    else if (lines[i].ToUpper().Trim() == "INFO")
                    {
                        ...
                        {
                            Department = infos[0].Trim(),
                            Mode = infos[1].Trim(),
                            Field = infos[2].Trim(),
                            Degree = infos[3].Trim(),
                            Semester = Convert.ToInt32(infos[4].Trim()),
                            Year = Convert.ToInt32(infos[5].Trim()),
                        };
                    ...
                    else if (dni.Contains(lines[i].ToUpper().Trim()))
                    {
                        ...
                        for (int j = 0; j < dayLines.Length;)
                        {
                            ...
                            TimetableDepartment k = MakeTimetableDepartment(currentInfo.Department, currentInfo.Mode, currentInfo.Field, currentInfo.Semester, currentInfo.Degree, currentInfo.Year, day, ev);
        ...
        {
            t.Departments = t.Departments.OrderBy(i => i.Department).ToList();
            foreach (var item in t.Departments)
            {
                item.Modes = item.Modes.OrderBy(i => i.Mode).ToList();
        ...
        public static TimetableDepartment MakeTimetableDepartment(string department, string mode, string field, int semester, string degree, int year, DayOfWeek dayOfWeek, TimetableEvent e)
        {
            return new TimetableDepartment()
            {
        ...
        public static Timetable MergeToTimetable(Timetable t, TimetableDepartment d)
        {
            //adding event to the timetable tree
            if (t.Departments.Exists(e => e.Department == d.Department))
            {
                TimetableDepartment nextLayerTimetable = t[d.Department].First();
                TimetableMode nextLayerK = d.Modes[0];
                if (nextLayerTimetable.Modes.Exists(e => e.Mode == nextLayerK.Mode))
                {
                    TimetableMode nextLayerTimetable2 = nextLayerTimetable[nextLayerK.Mode].First();
                    TimetableField nextLayerK2 = nextLayerK.Fields[0];
                ...
                {
                    nextLayerTimetable.Modes.Add(nextLayerK);
                }
            }
            else
            {
